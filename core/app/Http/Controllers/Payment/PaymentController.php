...
class PaymentController extends Controller
{
    ...
    public function saveOrder($request, $package_inputs, $paymentStatus, $gtype = 'online')
    {
        ...
        $sub->is_upgrade = $request->is_upgrade;
        ...
        // dd($package->toArray());
        $document_file = [];
        if (!auth()->user()->subscription || (auth()->user()->subscription && auth()->user()->subscription->current_package_id != $package->id)) {
            if ($package->document_copy) {
                $document_copy = explode(',', $package->document_copy);
                $dir = 'assets/front/document/';
                if ($sub->document_file) {
                    foreach (json_decode($sub->document_file, true) as $key => $value) {
                        @unlink($dir . $value[$document_copy[$key]]);
                    }
                }

                @mkdir($dir, 0775, true);
                foreach (json_decode($package->document_copy_field, true) as $key => $value) {
                    $receipt = uniqid() . '.' . $request->file($value)->getClientOriginalExtension();
                    $request->file($value)->move($dir, $receipt);

                    $document_file[] = [trim($document_copy[$key]) => $receipt];
                }
            }
        }
        $sub->document_file = json_encode($document_file, true);
        ...
        PackageHelper::createPayment($package, $method, $total_price, $sub, $bex->base_currency_text);
    ...
    {
        $user = Auth::user();
        $activeSub = Subscription::where('user_id', $user->id)->where('status', 1);
