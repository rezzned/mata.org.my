...
class ProductOrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        if ($po->user) {
            $title = __('Your order is ') . $request->order_status . ($po->rejected_note ? '. Reason:' . $po->rejected_note : '');
            $po->user->notify(new OrderStatusNotify($title, $po));
        }
    ...
    public function invoice($id)
    {
        ...
        $invoice_folder = 'assets/front/invoices/product/';
        $path = $invoice_folder . $order->invoice_number;

        if (!$order->invoice_number || !file_exists(root_path($path))) {
            $fileName = productCreatePdf($order);
            $path = $invoice_folder . $fileName;
        }
