...
class GatewayController extends Controller
{
    public function index()
    {
        ...
        $data['razerms'] = PaymentGateway::find(20);
    ...
    public function razerMSUpdate(Request $request)
    {
        $razerms = PaymentGateway::find(20);
        $razerms->status = $request->status;

        $information = [];
        $information['sandbox'] = $request->sandbox == 1 ? '1' : '0';
        $information['merchantid'] = $request->merchantid;
        $information['key'] = $request->key;
        $information['secret'] = $request->secret;
        $information['text'] = "Pay via your RazerMS account.";
        $razerms->name = $request->name;

        $razerms->information = json_encode($information);

        $razerms->save();

        $request->session()->flash('success', "RazerMS informations updated successfully!");
