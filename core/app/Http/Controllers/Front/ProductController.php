...
class ProductController extends Controller
{
    ...
    public function cart()
    {
        ...
        if (Session::has('event_cart')) {
            $event_cart = Session::get('event_cart');
        } else {
            $event_cart = null;
        }
        ...
        return view('front.product.cart', compact('cart', 'version', 'event_cart'));
    }
        ...
        if (strpos($id, ',,,') == true) {
            ...
            if (Auth::check()) {
                if (Auth::user()->is_member('associate_member')) {
                    $price = $product->assoc_price;
                }
                if (Auth::user()->is_member('standard_member')) {
                    $price = $product->stand_price;
                }
            }
    ...
    public function updatecart(Request $request)
    {
        ...
        $econtrol = new EventController;
        $res = $econtrol->updateCart($request);
        if (!$res[0]) {
    ...
    {
        $coupon = Coupon::where(['code' => $request->coupon, 'coupon_type' => 'publication']);
        $bex = BasicExtra::first();
