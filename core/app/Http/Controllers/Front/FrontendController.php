...
class FrontendController extends Controller
{
    ...
    public function packages()
    {
        ...
        $data['categories'] = PackageCategory::where('language_id', $currentLang->id)
            ->where('status', 1)
            ->whereHas('packageList', function ($q) {
                $q->where('status', 1);
            })
    ...
    public function events(Request $request)
    {
        ...
        $data['events'] = Event::with('eventCategories')
            ...
            ->when($request->date, function ($q) use ($request) {
                return $q->whereDate('date', date('Y-m-d', strtotime($request->date)));
            }, function ($q) {
                $q->where('date', '>', today());
            })
            ->where('lang_id', $currentLang->id)
    ...
    public function eventDetails($slug)
    {
        ...
        $version = $be->theme_version;
        $data['event_ticket'] = $event->eventTicket->unique('type');
        $data['ticket_data'] = eventTicket($data['event_ticket']);
    ...
    public function packageorder(Request $request, $id)
    {
        ...
        if ($bex->recurring_billing == 1) {
            ...
            if ($data['package']->type != 'standard_member') {
                # code...
                if (!empty($sub->next_package_id) && in_array($data['package']->type, ['associate_to_standard_member', 'standard_member'])) {
                    if (round(Auth::user()->cpd_point) < 20 || Carbon::parse($sub->created_at)->age < 3) {
                        Session::flash('error', 'You can\'t change to standard member. Because your CPD must be 20 or above and min 3 year of associate membership');
                        return redirect()->action([FrontendController::class, 'packages']);
                    }
                }
            }

            $subcription = auth()->user()->subscription ? auth()->user()->subscription->current_package : null;

            if (!$subcription && $data['package']->type == 'associate_to_standard_member') {
                Session::flash('error', 'You need to be an associate member to upgrade to standard member');
                return redirect()->action([FrontendController::class, 'packages']);
            }
            if (
                $subcription && $subcription->type == 'standard_member'
                && (in_array($data['package']->type, ['associate_to_standard_member', 'associate_member']))
            ) {
                Session::flash('error', 'You already standard member');
                return redirect()->action([FrontendController::class, 'packages']);
            }
    ...
    public function submitorder(Request $request)
    {
        ...
        $payment_controller->saveOrder($request, $package_inputs, 0);

        return PackageHelper::submitOrder($request->package_id, $request, $be, $fields);
    }
    ...
    public function loadpayment($slug, $id)
    {
        ...
        if ($data['pay_id'] != 0 && $data['payment'] != "offline") {
            ...
            $data['client_keys'] = $gateway->convertAutoData();
        } else {
