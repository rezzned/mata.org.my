...
@section('content')
    <div class="page-header">
        <h4 class="page-title">Training Bookings</h4>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="card-title d-inline-block">Training Bookings</div>
                        </div>
                        <div class="col-lg-4 offset-lg-4 mt-2 mt-lg-0">
                            ...
                            <form action="{{url()->current()}}" class="d-inline-block float-right">
                                <input class="form-control" type="text" name="search" placeholder="Search by Transaction ID" value="{{request()->input('search') ? request()->input('search') : '' }}">
                            </form>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped mt-3">
                                        ...
                                        @foreach ($events as $key => $event)
                                            <tr>
                                                ...
                                                <td>
                                                    @if($event->status != "Pending")
                                                        ...
                                                        @endif
                                                    @else
                                                        <form id="statusForm{{$event->id}}" class="d-inline-block" action="{{route('admin.event.payment.log.update')}}" method="post">
                                                            @csrf
                                                            <input type="hidden" name="id" value="{{$event->id}}">
                                                            <select class="order_status form-control form-control-sm
                                                            ...
                                                            <!-- Modal -->
                                                            <div class="modal fade" id="rejected_modal_{{$event->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                                <div class="modal-dialog modal-dialog-centered">
                                                                    <div class="modal-content">
                                                                        <div class="modal-header">
                                                                        <h5 class="modal-title" id="exampleModalLabel">Booking rejected status note</h5>
                                                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                                            <span aria-hidden="true">&times;</span>
                                                                        </button>
                                                                        </div>
                                                                        <div class="modal-body">
                                                                            <textarea name="rejected_note" class="form-control" rows="5"></textarea>
                                                                        </div>
                                                ...
                                                <td>
                                                    @if (!empty($event->receipt))
                                                        <a class="btn btn-sm btn-info" href="#" data-toggle="modal" data-target="#receiptModal{{$event->id}}">Show</a>
                                                    @else
                                                        -
                                                    @endif
                                                </td>
                                                <td>
                                                    @if (!empty($event->name !== "anonymous"))
                                                        <a class="btn btn-sm btn-info" href="#" data-toggle="modal" data-target="#detailsModal{{$event->id}}">Detail</a>
                                                    @else
                                                        -
                                                    @endif
                                            ...
                                            <div class="modal fade" id="receiptModal{{$event->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <img src="{{asset('assets/front/img/events/receipt/' . $event->receipt)}}" alt="Receipt" width="100%">
                                                        </div>
                                                        <div class="modal-footer">
                                                            <a href="{{asset('assets/front/img/events/receipt/' . $event->receipt)}}" class="btn btn-success" download="{{ $event->receipt }}">Download</a>
                                                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="modal fade" id="detailsModal{{$event->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog" role="document">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            <p class="d-inline-block mb-0">{{$event->paymentGateway->name ?? $event->payment_method}}</p>
                                                            <hr>
                                                            @if ($event->status == 'Success')
                                                            <label>Invoice:</label>
                                                            <p class="d-inline-flex mb-0">
                                                                @if ($fileClass::exists($pdf_path($event->invoice)))
                                                                <a href="{{asset('assets/front/invoices/' . urlencode($event->invoice))}}" download="Invoice-{{$event->event->slug}}.pdf" class="btn btn-primary btn-sm mx-2">
                                                                    {{__('Download Invoice')}}
                                                                </a>
                                                                <form action="{{ route('admin.event.ticket-regenerate', [$event->id, $event->transaction_id]) }}" method="post">@csrf
                                                                    <button class="btn btn-danger btn-sm mx-2" onclick="return confirm('Are you sure to regenerate the invoice?')">{{__('Regenerate')}}</button>
                                                                </form>
                                                                @else
                                                                <form action="{{ route('admin.event.ticket-generate', [$event->id, $event->transaction_id]) }}" method="post">@csrf
                                                                    <button class="btn btn-primary btn-sm mx-2">{{__('Generate & Download Invoice')}}</button>
                                                                </form>
                                                                @endif
                                                            </p>
                                                            <hr>
