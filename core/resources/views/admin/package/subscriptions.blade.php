                      ...
                      @foreach ($subscriptions as $key => $sub)
                        <tr>
                          ...
                          <td>
                              {{$sub->name}}
                              @if (!$sub->user)
                              <span title="User not available">
                                <svg width="14" height="14" fill="currentColor" class="bi bi-slash-circle ml-2" viewBox="0 0 16 16">
                        ...
                        @if (request()->input('type') == 'all')
                          <td class="text-center">
                            <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#dateModal{{$sub->id}}">
                                Change
                            </button>
                            <div class="modal fade" id="dateModal{{$sub->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                              <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Change Membership Date</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                      <span aria-hidden="true">&times;</span>
                                    </button>
                                  </div>
                                  <form action="{{ route('admin.subscriptions.change.date') }}" method="post">
                                    @csrf
                                    <div class="modal-body">
                                      <div class="row px-5">
                                        <div class="col-sm-12 my-2 text-left">
                                            <label for="start_date_{{ $sub->id }}" class="mb-1">{{__('Associate Start Date')}}</label>
                                            <input type="date" name="start_date" id="start_date_{{ $sub->id }}" class="form-control" value="{{ Carbon\Carbon::parse($sub->user->associate_member_start_date ?? $sub->created_at)->format('Y-m-d') }}">
                                        </div>
                                        <div class="col-sm-12 my-2 text-left">
                                            <label for="expire_date_{{ $sub->id }}" class="mb-1">{{__('Expire Date')}}</label>
                                            <input type="date" name="expire_date" id="expire_date_{{ $sub->id }}" class="form-control" value="{{ Carbon\Carbon::parse($sub->expire_date)->format('Y-m-d') }}">
                                        </div>
                                      </div>
                                    </div>
