...
<a data-toggle="collapse" href="#pages">
    ...
    <p>Custom Pages {{-- <spanclass="badgebadge-dangerp-1sidenav-badge">Pagebuilder</span> --}}</p>
...
<ul class="nav nav-collapse">
    ...
    <li class="@if(request()->path() == 'admin/packages') active
        ...
        <a href="{{route('admin.package.index') . '?language=' . $default->code}}">
            <span class="sub-item">Membership</span>
        </a>
    </li>
        ...
        <ul class="nav nav-collapse subnav">
            <li class="@if(request()->routeIs('admin.subscriptions') && request()->input('type') == 'all') active @endif">
                <a href="{{route('admin.subscriptions', ['type' => 'all'])}}">
                    <span class="sub-item">All Registration</span>
                </a>
            </li>
            ...
            <li class="@if(request()->routeIs('admin.subscriptions') && request()->input('type') == 'expired') active @endif">
                <a href="{{route('admin.subscriptions', ['type' => 'expired'])}}">
                    <span class="sub-item">Expired Registration</span>
                </a>
            </li>
        </ul>
    </div>
...
@php
    $subscriptions_count = App\Subscription::whereNotNull('pending_package_id')->count();
@endphp
<li class="@if(request()->routeIs('admin.subscriptions') && request()->input('type') == 'request') active @endif">
    <a href="{{route('admin.subscriptions', ['type' => 'request'])}}">
        <span class="sub-item">Membership Requests</span>
        <span class="badge badge-info float-right">{{ $subscriptions_count ?: '' }}</span>
    </a>
</li>
@endif
...
@elseif(request()->path() == 'admin/publication/coupon') show
...
@if ($bex->catalog_mode == 0)
<li class="
@if(request()->routeIs('admin.coupon.index',['coupon_type' => 'publication'])) active
@elseif(request()->routeIs('admin.coupon.edit',['coupon_type' => 'publication'])) active
@endif">
<a href="{{route('admin.coupon.index',['coupon_type' => 'publication'])}}">
...
<li class="submenu">
    ...
    <span class="sub-item">Manage Publication</span>
    <span class="caret"></span>
</a>
<div class="collapse
@if(request()->path() == 'admin/category') show
...
<a href="{{route('admin.product.type')}}">
    <span class="sub-item">Add Publication</span>
</a>
</li>
<li class="
...
@php
    $side_order = App\ProductOrder::whereIn('order_status',['pending','processing'])->get();
    $pending_orders = $side_order->where('order_status', 'pending')->count();
    $processing_orders = $side_order->where('order_status', 'processing')->count();
@endphp
@if ($bex->catalog_mode == 0)
<li class="submenu">
...
<ul class="nav nav-collapse subnav">
    ...
    <li class="@if(request()->path() == 'admin/product/pending/orders') active @endif">
        <a href="{{route('admin.pending.product.orders')}}">
            <span class="sub-item">Pending Orders</span>
            <span class="badge badge-info float-right">{{ $pending_orders ?: '' }}</span>
        </a>
    </li>
    <li class="@if(request()->path() == 'admin/product/processing/orders') active @endif">
        <a href="{{route('admin.processing.product.orders')}}">
            <span class="sub-item">Processing Orders</span>
            <span class="badge badge-info float-right">{{ $processing_orders ?: '' }}</span>
        </a>
    </li>
...
@elseif(request()->path() == 'admin/training/coupon') active
@endif">
...
@elseif(request()->path() == 'admin/training/coupon') show
...
<ul class="nav nav-collapse">
    ...
    @if ($bex->catalog_mode == 0)
    <li class="
    @if(request()->routeIs('admin.coupon.index',['coupon_type' => 'training'])) active
    @elseif(request()->routeIs('admin.coupon.edit',['coupon_type' => 'training'])) active
    @endif">
    <a href="{{route('admin.coupon.index',['coupon_type' => 'training'])}}">
        <span class="sub-item">Coupons</span>
    </a>
    </li>
    @endif
    ...
    <a href="{{route('admin.event.index') . '?language=' . $default->code}}">
        <span class="sub-item">All Training Events</span>
    </a>
</li>
@php
    $event_booking = App\EventDetail::where('status','Pending')->count();
@endphp
...
@elseif(request()->routeIs('admin.membership-tracker')) show
@elseif(request()->routeIs('admin.req-cpd-external')) show
...
<ul class="nav nav-collapse">
    ...
    @endif">
    <a href="{{route('admin.membership-tracker')}}">
        <span class="sub-item">CPD Points</span>
    </a>
</li>
@php
    $external_cpd_count = App\CpdExternalPoint::where('status', 0)->with('user')->count();
@endphp
</li>
    <li class="
    @if(request()->routeIs('admin.req-cpd-external')) active
    @endif">
    <a href="{{route('admin.req-cpd-external')}}">
        <span class="sub-item">External CPD</span>
        <span class="badge badge-info float-right">{{ $external_cpd_count ?: '' }}</span>
    </a>
</li>
...
@elseif(request()->path() == 'admin/banner') active
@endif">
...
@elseif(request()->path() == 'admin/banner') show
@endif" id="basic">
<ul class="nav nav-collapse">
    ...
    <li class="@if(request()->path() == 'admin/banner') active @endif">
        <a href="{{route('admin.banner.index',['language'=>'en'])}}">
