            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-lg-6 offset-lg-3 mt-lg-0 mt-2">
                            <button
                                    class="btn btn-danger btn-sm d-none bulk-delete float-right"
                                    data-href="{{ route('admin.course.purchaseBulkOrderDelete') }}"><i class="flaticon-interface-5"></i> Delete</button>
                                    ...
                                    <table class="table-striped mt-3 table">
                                        ...
                                        <tbody>
                                            @foreach ($purchases as $purchase)
                                                <tr>
                                                    <td>
                                                        <input type="checkbox" class="bulk-check" data-val="{{ $purchase->id }}">
                                                    </td>
                                                    <td>{{ $purchase->order_number }}</td>
                                                    <td>{{ strlen($purchase->course->title ?? 'Deleted') > 25 ? mb_substr($purchase->course->title ?? 'Deleted', 0, 25, 'utf-8') . '...' : $purchase->course->title ?? 'Deleted' }}</td>
                                                    <td>{{ $purchase->user ? $purchase->user->username : 'N/A' }}</td>
                                                    <td>
                                                        ...
                                                        @if ($purchase->payment_status == 'Pending')
                                                            <form action="{{ route('admin.course.purchasePaymentStatus') }}" id="paymentStatusForm{{ $purchase->id }}" method="POST">
                                                                ...
                                                                <select class="{{ strtolower($purchase->payment_status) == 'completed' ? 'bg-success' : 'bg-danger' }} form-control-sm border-0 text-white" name="payment_status"
                                                        ...
                                                        @else
                                                            <span class="{{ strtolower($purchase->payment_status) == 'completed' ? 'badge badge-success' : 'badge badge-danger' }}">
                                                                {{ strtolower($purchase->payment_status) == 'completed' ? 'Completed' : 'Cancelled' }}</span>
                                                        @endif
                                                    </td>
                                                    <td>
