...
@section('pagename')
 - {{__('Publication')}} - {{convertUtf8($product->title)}}
@endsection
            ...
            <div class="col-lg-6">
                <div class="product-details-content ml-60 mt-30">
                    <div class="product-details-content-item">
                        ...
                        @if ($bex->catalog_mode == 0)
                            <span>
                                @auth
                                    ...
                                    {{currency_format($product->assoc_price)}}
                                    @elseif (auth()->user()->is_member('standard_member'))
                                    {{currency_format($product->stand_price)}}
                                    @else
                                    {{currency_format($product->current_price)}}
                                    @endif
                                @else
                                    {{currency_format($product->current_price)}}
                                @endauth
                        ...
                        @endif
                        <p><strong>Postal Fee : </strong> <strong>{{currency_format($product->postal_fee)}}</strong></p>
                                ...
                                @if (count($reviews) > 0)
                                    ...
                                    <div class="shop-review-user">
                                        @if (strpos($review->user->photo, 'facebook') !== false || strpos($review->user->photo, 'google'))
                                            <img class="lazy" data-src="{{$review->user->photo ? $review->user->photo : asset('assets/front/img/user/profile-img.png')}}" alt="user image" width="60">
                                        @else
...
<div class="product-items">
    <div class="container">
        <div class="row">
            <div class="col-lg-6">
                <span class="section-title">{{__('Related Publications')}}</span>
                <h2 class="section-summary"{{__('>Have a look at the finlance latest Publication')}}></h2>
            </div>
        </div>
