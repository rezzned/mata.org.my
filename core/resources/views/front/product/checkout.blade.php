        ...
        <div class="bottom">
            <div class="container">
                <div class="row">
                    ...
                    <div class="col-xl-6 col-lg-6 col-md-12 col-sm-12">
                        <div class="table">
                            ...
                            <table class="cart-table">
                                <thead class="cart-header">
                                    <tr>
                                        <th class="product-column">{{__('Publication')}}</th>
                                        <th>&nbsp;</th>
                                        <th>{{__('Quantity')}}</th>
                                ...
                                <tbody>
                                    ...
                                    @php
                                        ...
                                        $total += $item['price'] * $item['qty'] + $product->postal_fee * $item['qty'];
                                        $shipping_charge += $product->shipping_price;
                                        $postal_fee += $product->postal_fee * $item['qty'];
                                    @endphp
                                    <tr>
                    ...
                    <input type="hidden" name="postal_fee" value='{{ $postal_fee }}' />
                        ...
                        <div class="cart-total">
                            ...
                            <div id="cartTotal">
                                <ul class="cart-total-table">
                                    ...
                                    <li class="clearfix">
                                        <span class="col col-title">{{ __('Postal Fee') }}
                                            <span class="text-danger">(<i class="fas fa-plus"></i>)</span>
                                        </span>
                                        <span class="col">
                                            {{ $bex->base_currency_symbol_position == 'left' ? $bex->base_currency_symbol : '' }}<span
                                            id="postal_fee">{{ $postal_fee }}</span>{{ $bex->base_currency_symbol_position == 'right' ? $bex->base_currency_symbol : '' }}
                                        </span>
                                    </li>
                                    ...
                                    @php
                                        $scharge = 0;
                                    @endphp
                                    @if (!onlyDigitalItemsInCart() && $shipping_charge > 0)
                                        @php
                                            $scharge = round($shipping_charge,2);
                                        @endphp
                                        <li class="clearfix">
                                    ...
                                    <li class="clearfix">
                                        ...
                                        <span class="col">
                                            {{$bex->base_currency_symbol_position == 'left' ? $bex->base_currency_symbol : ''}}<span data="{{ cartSubTotal() + $scharge + tax() + $postal_fee }}" class="grandTotal">{{ cartSubTotal() + $scharge + tax() + $postal_fee }}</span>{{$bex->base_currency_symbol_position == 'right' ? $bex->base_currency_symbol : ''}}</span>
                                    </li>
                                </ul>
                            </div>
                            ...
                            <div class="payment-options">
                                ...
                                <div class="placeorder-button text-left">
                                    <button {{$cart ? '' : 'disabled' }}  class="main-btn submit_btn_load" type="submit"><span class="btn-title">{{__('Place Order')}}</span></button>
                                </div>
                            </div>
...
<script>
            ...
            function(data) {
                ...
                if (data.status == 'success') {
                    ...
                    $("#cartTotal").load(location.href + " #cartTotal", function() {
                        ...
                        let postal_fee = parseFloat($("#postal_fee").text());
    ...
    $(document).on('click', '.shipping-charge', function(){
        ...
        postal_fee = parseFloat($('#postal_fee').text());

        let shipCharge = parseFloat($(this).attr('data'));

        shipping = parseFloat(shipCharge);
        total = parseFloat(parseFloat(grantotal) + parseFloat(shipCharge) + parseFloat(postal_fee));
