                      ...
                      <div class="price">
                        @auth
                            @if (auth()->user()->is_member('associate_member'))
                                <span>{{ empty($course->assoc_price) ? __('Free') : currency_format($course->assoc_price) }}</span>
                            @elseif (auth()->user()->is_member('standard_member'))
                                <span>{{ empty($course->stand_price) ? __('Free') : currency_format($course->stand_price) }}</span>
                            @else
                                <span>{{ empty($course->current_price) ? __('Free') : currency_format($course->current_price) }}</span>
                            @endif
                        @else
                            <span>{{ empty($course->current_price) ? __('Free') : currency_format($course->current_price) }}</span>
                        @endauth
                        @if (!empty($course->previous_price))
                            <span class="pre-price"><del>{{ currency_format($course->previous_price) }}</del></span>
                        @endif
                      </div>
                    </div>
                    <div class="course-meta">
