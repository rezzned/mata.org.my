          ...
          @endif
            ...
            @auth
                @if (auth()->user()->is_member('associate_member'))
                    <span class="off-price">{{ empty($course_details->assoc_price) ? __('Free') : currency_format($course_details->assoc_price) }}</span>
                @elseif (auth()->user()->is_member('standard_member'))
                    <span class="off-price">{{ empty($course_details->stand_price) ? __('Free') : currency_format($course_details->stand_price) }}</span>
                @else
                    <span class="off-price">{{ empty($course_details->current_price) ? __('Free') : currency_format($course_details->current_price) }}</span>
                @endif
            @else
                <span class="off-price">{{ empty($course_details->current_price) ? __('Free') : currency_format($course_details->current_price) }}</span>
            @endauth
            @if (!empty($course_details->previous_price))
            <span class="main-price">{{ currency_format($course_details->previous_price) }}</span>
            @endif
            </p>
            ...
            <div class="d-flex flex-row">
              ...
              <div class="form-group button mb-20">
                <button type="submit" id="enrollBtn"
                  class="main-btn submit_btn_load {{ $course_details->current_price == null ? 'd-none' : '' }}">{{ __('Enroll Now') }}</button>
              </div>
              <div class="form-group button mb-20">
            ...
            <div class="row mt-3 d-none" id="razermsTab">
                <div class="mb-4">
                    <p class="mb-3">
              ...
              <div class="shop-review-area">
                ...
                <div class="mb-5">
                  ...
                  <div class="review_user">
                    ...
                    <img class="lazy"
                      data-src="{{$review->reviewByUser->photo ? $review->reviewByUser->photo : asset('assets/front/img/user/profile-img.png')}}"
                      alt="user image">
                    @else
                    <img class="lazy"
  ...
  $(document).ready(function() {
    ...
    //    console.log(paymentGateway, type);
        ...
        if (type == 'offline') {
            ...
            } else if (paymentGateway == 'razerms') {
                $('#paymentGatewayForm').attr('action', "{{ route('course.payment.razerms') }}");
     ...
     $(document).on('change', '#paymentType', function() {
       ...
       if (gatewayName == 'razerms') {
         $('#razermsTab').removeClass('d-none');
         $('#razermsTab input').removeAttr('disabled');
       } else {
         $('#razermsTab input').attr('disabled', true);
         $('#razermsTab').addClass('d-none');
       }
