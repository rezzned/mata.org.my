...
<html lang="en">
   <head>
    ...
    {{-- <link href="{{url('/')}}/assets/front/css/common-base-color.php?color={{$base_color}}" rel="stylesheet"> --}}
    <!-- base color change -->
    {{-- <link href="{{url('/')}}/assets/front/css/base-color.php?color={{$base_color}}" rel="stylesheet"> --}}
    @includeif('inc.common_base_color',['color' => $base_color])
    @includeif('inc.base_color',['color' => $base_color,'color1'=>'','navbar_color'=>$navbar_bg_color])

      {{-- @if ($be->theme_version == 'dark')
        <!-- dark version css -->
        <link rel="stylesheet" href="{{asset('assets/front/css/dark.css')}}">
        <!-- dark version base color change -->
        <link href="{{url('/')}}/assets/front/css/dark-base-color.php?color={{$base_color}}" rel="stylesheet">
      @endif --}}
      ...
      @endif
      <script src="{{ asset('assets/front/js/dayjs.min.js') }}"></script>
      <!-- jquery js -->
               ...
               <div class="row">
                  ...
                  <div class="col-lg-6 {{$rtl == 1 ? 'text-left' : 'text-right'}}">
                    ...
                    <ul class="social-links">
                        ...
                        <li><a class="text-dark" target="_blank" href="{{$social->url}}"><i class="{{$social->icon}}"></i></a></li>
                        @endforeach
                    </ul>
                    @endif
                     ...
                     @if (!empty($currentLang) && count($langs) > 1)
                       <div class="language">
                          <a class="language-btn text-dark" href="#"><i class="flaticon-worldwide"></i> {{convertUtf8($currentLang->name)}}</a>
                          <ul class="language-dropdown">
                            @foreach ($langs as $key => $lang)
                            <li><a  class="text-dark" href='{{ route('changeLanguage', $lang->code) }}'>{{convertUtf8($lang->name)}}</a></li>
                            @endforeach
                          </ul>
                       </div>
                     @endif
                     ...
                     @guest
                        @if ($bex->is_user_panel == 1)
                            <ul class="login">
                                <li><a class="text-dark" href="{{route('user.login')}}">{{__('Login')}}</a></li>
                            </ul>
                            <ul class="login border-left pl-2">
                                <li><a class="text-dark" href="{{route('user-register')}}">{{__('Register')}}</a></li>
                            </ul>
                        @endif
                    @endguest
                     @auth
                     <div class="language dashboard">
                         <a class="language-btn text-dark" href="#">
                             <i class="far fa-user"></i> {{Auth::user()->username}}
                         </a>
                         <ul class="language-dropdown">
                             <li>
                                 <a  class="text-dark" href="{{route('user-dashboard')}}">{{__('Dashboard')}}</a>
                             </li>
                             <li>
                                <a  class="text-dark" href="{{ route('user-notification') }}">
                                    {{ __('My Notifications') }}
                                </a>
                            </li>

                             @if ($bex->recurring_billing == 1)
                                 <li><a class="text-dark" href="{{route('user-packages')}}">{{__('Membership')}}</a></li>
                             @endif

                             @if ($bex->is_shop == 1 && $bex->catalog_mode == 0)
                                 <li><a class="text-dark" href="{{route('user-orders')}}">{{__('Publication Orders')}} </a></li>
                             @endif
                             ...
                             @if ($bex->is_event == 1)
                             <li>
                                 <a href="{{route('user-events')}}" >{{__('Training Bookings')}}</a>
                             </li>
                             @endif
            ...
            <div id="cartIconWrapper">
                <a class="d-block" id="cartIcon" href="{{route('front.cart')}}">
                    <div class="cart-length">
                        ...
                        <span class="length">{{CartItemTotal()}} {{__('ITEMS')}}</span>
                    </div>
    ...
    <script src="{{asset('assets/front/js/common-main.js')}}" defer></script>
        ...
        <script>
            ...
            $(document.body).on('submit', 'form', function(event) {
                let submit_btn = $(this).find('.submit_btn_load');
                if (!submit_btn.hasClass('disabled')) {
                    submit_btn.prop('disabled',true);
                    submit_btn.addClass('disabled').append('<div class="ml-2 spinner-border spinner-border-sm" role="status"><span class="sr-only"></span></div>');
                }
            });
        </script>
